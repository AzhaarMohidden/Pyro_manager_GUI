...
def Search_ports():
    ...
    # COM_options.config(root, variable, *available_ports, command=portss)
...
def Connect():
    ...
    try:
        ...
        # term_listbox.insert(l, term)
        print_term(term)
        Connect_result = send_data(3)
        status = 1
        if Connect_result == "3":
            print("Connected Successfully")
            # Button_Open = Button(root, text= "Open Port", command = Connect, bg = '#00FF00' )
...
# Button_Close = Button(root, text= "Close Port", command = Coms_end, padx = 20)
# Button_Open = Button(root, text="Open", command = open_ports)
